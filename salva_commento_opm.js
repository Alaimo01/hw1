    ...
    if(!json){
       ...
       for(let i = 0 ; i < json.length ; i++){
        ...
        contenitore1.appendChild(contenitore);
               ...
               const datas = await fetch('opm.php?utente='+json[i].utente).then(onResponse).then(onjsonelimina);
       ...
       for(let j = 0; j<json.length;j++){
        fetch('opm.php?id2='+ json[j].id_commento).then(onResponse1).then(onjsonpreferiti1);
       }
       
       /*for(let i = 0; i<json.length;i++){
        for(let j = 0; j<json.length;j++){
            if(json[i].id_commento === json[j].id_commento){
                const immagine = document.createElement('img');
                immagine.classList.add('preferiti');
...
function cancella_commento(event){
    ...
    fetch('opm.php?cancella1=' + encodeURIComponent(comm1)+'&chat=opm').then(onResponse).then(onjsonelimina);
    console.log(comm1);
...
function invia_testo(event){
    ...
    fetch('opm.php?commento='+encodeURIComponent(comm.value)+'&chat=opm').then(onResponse).then(onjson);
...
function metti_preferiti(event){
    ...
    fetch('opm.php?preferiti=' + encodeURIComponent(comm1)+'&chat=opm & utente='+ encodeURIComponent(utente)+'&id='+id).then(onResponsepreferiti).then(onjsonpreferiti);
    console.log(comm1);
    console.log(utente);
    console.log(id);
}
...
function togli_preferiti(event){
    ...
    const id = event.currentTarget.parentNode.querySelector('.preferiti').id;
    fetch('opm.php?togli_id='+id).then(onResponserimozione).then(onjsontoglipreferiti);
    console.log(id);
}

fetch('opm.php?stampa=ciao').then(onResponse).then(onjson);
