    ...
    if(!json){
       ...
       for(let i = 0 ; i < json.length ; i++){
        ...
        contenitore1.appendChild(contenitore);
               ...
               const datas = await fetch('dragonball.php?utente='+json[i].utente).then(onResponse).then(onjsonelimina);
       ...
       for(let j = 0; j<json.length; j++){   
        fetch('dragonball.php?id2='+ json[j].id_commento).then(onResponse).then(onjsonpreferiti1);
...
function cancella_commento(event){
    ...
    fetch('dragonball.php?cancella1=' + encodeURIComponent(comm1)+'&chat=dragonball').then(onResponse).then(onjsonelimina);
    console.log(comm1);
...
function invia_testo(event){
    ...
    fetch('dragonball.php?commento='+encodeURIComponent(comm.value)+'&chat=dragonball').then(onResponse).then(onjson);
...
function metti_preferiti(event){
    ...
    fetch('dragonball.php?preferiti=' + encodeURIComponent(comm1)+'&chat=dragonball & utente='+ encodeURIComponent(utente)+'&id='+id).then(onResponsepreferiti).then(onjsonpreferiti);
    console.log(comm1);
    console.log(utente);
    console.log(id);
}
...
function togli_preferiti(event){
    ...
    const id = event.currentTarget.parentNode.querySelector('.preferiti').id;
    fetch('dragonball.php?togli_id='+id).then(onResponserimozione).then(onjsontoglipreferiti);
    console.log(id);
}

fetch('dragonball.php?stampa=ciao').then(onResponse).then(onjson);
