...
if(isset($_GET['stampa'])){
    ...
    $query = "SELECT * FROM commento_naruto";
...
if(isset($_GET['cancella1'])){
    ...
    $query = "DELETE FROM commento_naruto where commento = '$commento'";
    $res = mysqli_query($conn, $query);
    if ($res) {
...
if(isset($_GET['commento'])){
    ...
    $query = "SELECT * FROM commento_naruto";
    $insertQuery = "INSERT INTO commento_naruto (utente,commento,data) VALUES (\"$utente\",\"$commento\",CURDATE())";
...
<head>
    <title>NARUTO</title>
    <link rel="stylesheet" href="naruto.css?ts=<?=time()?>&quot"/>
    <script src="salva_commento_naruto.js" defer="true"></script>
