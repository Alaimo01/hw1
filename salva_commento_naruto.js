    ...
    if(!json){
       ...
       for(let i = 0 ; i < json.length ; i++){
        ...
        contenitore1.appendChild(contenitore);
               ...
               const datas = await fetch('naruto.php?utente='+json[i].utente).then(onResponse).then(onjsonelimina);
       ...
       for(let j = 0; j<json.length; j++){   
        fetch('naruto.php?id2='+ json[j].id_commento).then(onResponse).then(onjsonpreferiti1);
...
function cancella_commento(event){
    ...
    fetch('naruto.php?cancella1=' + encodeURIComponent(comm1)+'&chat=naruto').then(onResponse).then(onjsonelimina);
    console.log(comm1);
...
function invia_testo(event){
    ...
    fetch('naruto.php?commento='+encodeURIComponent(comm.value)+'&chat=naruto').then(onResponse).then(onjson);
...
function metti_preferiti(event){
    ...
    fetch('naruto.php?preferiti=' + encodeURIComponent(comm1)+'&chat=naruto & utente='+ encodeURIComponent(utente)+'&id='+id).then(onResponsepreferiti).then(onjsonpreferiti);
    console.log(comm1);
    console.log(utente);
    console.log(id);
}
...
function togli_preferiti(event){
    ...
    const id = event.currentTarget.parentNode.querySelector('.preferiti').id;
    fetch('naruto.php?togli_id='+id).then(onResponserimozione).then(onjsontoglipreferiti);
    console.log(id);
}

fetch('naruto.php?stampa=ciao').then(onResponse).then(onjson);
